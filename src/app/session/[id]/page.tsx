...
import { SessionBackLabel, SessionEditLabel, SessionTotalLabel, SessionWeightLabel } from "./SessionDetailClient";

export const revalidate = 0;

function describeSetLine(s: { set_count: number | null; reps: number | null; duration_sec: string | null }): string {
...
export default async function SessionPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const exercisesWithSets = await Promise.all(
    exRows.map(async (se) => {
      ...
      const setRows = await db.select().from(sets).where(eq(sets.session_exercise_id, se.id)).orderBy(sets.position);
    ...
    <div style={{ minHeight: "100vh", background: "var(--bg)", color: "var(--text)", fontFamily: "var(--font)" }}>
      ...
      <div style={{ maxWidth: 680, margin: "0 auto", padding: "1.25rem 1rem 4rem" }}>
        ...
        <div style={{
          ...
          borderRadius: 8, padding: "1.25rem 1.25rem 1.25rem 1.5rem",
          borderLeft: "4px solid var(--lime)", marginBottom: "1rem",
        }}>
          <h1 style={{ color: "var(--lime)", fontWeight: 800, fontSize: "1.15rem", letterSpacing: "0.04em", textTransform: "uppercase", marginBottom: "0.35rem" }}>
            {sessionTitle}
          </h1>
          <div style={{ fontSize: "0.85rem", color: "var(--muted)", marginBottom: session.notes ? "0.75rem" : 0 }}>
            {formatDate(session.performed_on)}{" "}— <SessionWeightLabel value={session.bodyweight_kg} />
          </div>
          {session.notes && (
            <p style={{ marginTop: "0.5rem", fontStyle: "italic", color: "var(--muted)", fontSize: "0.9rem" }}>{session.notes}</p>
          )}
        </div>
        ...
        {exercisesWithSets.map((ex) => (
          <div key={ex.id} style={{
            ...
            <h3 style={{ color: "var(--lime)", fontWeight: 700, fontSize: "1rem", marginBottom: "0.6rem" }}>
            ...
            <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
              {ex.sets.map((s) => (
                <li key={s.id} style={{ marginBottom: s.note ? "0.55rem" : "0.35rem" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                    <span style={{ display: "inline-block", width: 6, height: 6, borderRadius: "50%", background: "var(--muted)", flexShrink: 0, marginTop: 2 }} />
                    <span style={{ fontSize: "0.95rem" }}>{describeSetLine(s)}</span>
                  </div>
                  {s.note && (
                    <div style={{ marginLeft: "1.1rem", paddingLeft: "0.6rem", borderLeft: "2px solid var(--border)", fontStyle: "italic", color: "var(--muted)", fontSize: "0.85rem", marginTop: "0.2rem" }}>
                      {s.note}
                    </div>
                  )}
                </li>
              ...
              {ex.sets.length > 0 && (
                <li style={{ marginTop: "0.4rem" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                    <span style={{ display: "inline-block", width: 6, height: 6, borderRadius: "50%", background: "var(--lime)", flexShrink: 0, marginTop: 2 }} />
                    <span style={{ fontSize: "0.95rem", fontWeight: 700, color: "var(--lime)" }}>
                      <SessionTotalLabel line={totalLine(ex.sets)} />
                    </span>
                  </div>
                </li>
              )}
              {ex.notes && (
                <li style={{ marginTop: "0.35rem" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                    <span style={{ display: "inline-block", width: 6, height: 6, borderRadius: "50%", background: "var(--muted)", flexShrink: 0, marginTop: 2 }} />
                    <span style={{ fontStyle: "italic", color: "var(--muted)", fontSize: "0.85rem" }}>{ex.notes}</span>
                  </div>
                </li>
              )}
            </ul>
